...
TRAIN_DIR=./data/2_code_review/before/train
VAL_DIR=./data/2_code_review/before/val
TEST_DIR=./data/2_code_review/before/test
DATASET_NAME=3_preprocessed_data_codereview
...
echo "pathの抽出 filepathがないのが気になる。"
...
# shellスクリプトで単語ごとの出現回数を算出していると思われる。
...
echo "${PYTHON} preprocess.py --train_data ${TRAIN_DATA_FILE} --test_data ${TEST_DATA_FILE} --val_data ${VAL_DATA_FILE} \
  --max_contexts ${MAX_CONTEXTS} --max_data_contexts ${MAX_DATA_CONTEXTS} --subtoken_vocab_size ${SUBTOKEN_VOCAB_SIZE} \
  --target_vocab_size ${TARGET_VOCAB_SIZE} --subtoken_histogram ${SOURCE_SUBTOKEN_HISTOGRAM} \
  --node_histogram ${NODE_HISTOGRAM_FILE} --target_histogram ${TARGET_HISTOGRAM_FILE} --output_name data/${DATASET_NAME}/${DATASET_NAME}
"
